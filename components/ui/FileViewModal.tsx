...
const FilePreview: React.FC<{ file: FileItem }> = ({ file }) => {
  ...
  const placeholderStyle = "flex flex-col items-center justify-center h-full w-full bg-gray-900/50 rounded-lg p-8 text-center text-gray-400";
  ...
  switch (extension) {
    ...
    default:
      return (
        <div className={placeholderStyle}>
            <File className="h-24 w-24 mb-4 text-gray-500" />
            <p className="font-semibold text-lg text-white">{file.filename}</p>
...
const FileViewModal: React.FC<FileViewModalProps> = ({ isOpen, onClose, file, user, onDownload }) => {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={title} className="max-w-4xl" backgroundClassName="bg-gray-800 border border-gray-700">
